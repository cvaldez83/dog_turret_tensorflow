...
OPTION_WINDUP_ON_OFF = "on" # set to "on" to enable the windup motor, else "off"
...
MAX_NUMBER_OF_WINDUP = 2000 # this is to restrict many times the gun winds up
...
TRIGGER_ANGLE_RANGE = 60 # degrees, desired full range of servo in degrees
GUN_WINDUP_TIMER = 10 # seconds, windup gun motor after this many seconds from initial detection
...
""" USER INPUT FOR TEXT PLACED ON BOUNDARY BOX USING OPENCV """
pos=(20,60)
    ...
    while GPIO.input(SWITCH_PIN) == 1 and COUNTER_NUMBER_OF_SHOTS <= MAX_NUMBER_OF_SHOTS: # run loop while switch is on
        ...
        for my_detection in my_detects.detections:
            ...
            if my_detection: #executes code if something was detected
                ...
                UL_target=(target_x_pixel-10,target_y_pixel-10) # Upper left of detected boundadry box
                LR_target=(target_x_pixel+10,target_y_pixel+10) # Lower right of detected boundary box
                im=cv2.rectangle(im,UL_target,LR_target,(255,0,0),2) #draw small square center of target
                shoot_at_angle = TURRET_ANGLE_RANGE*(center_mass / dispW) + LEFT_MOST_ANGLE
        ...
        if flagSpotted and time_at_first_spot !=0 and time_current > init_time_plus_G_W_T and time_current > time_to_cool_down  and COUNTER_NUMBER_OF_WINDUP <= MAX_NUMBER_OF_WINDUP:
        ...
        """ FIRE THE TURRET """
        if flagSpotted and flagWindingUp and COUNTER_NUMBER_OF_SHOTS <= MAX_NUMBER_OF_SHOTS:
